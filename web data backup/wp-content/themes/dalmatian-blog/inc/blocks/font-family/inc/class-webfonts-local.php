 ...
 */
final class Dalmatian_Blog_Webfonts_Local{
	...
	public function add_styles() {
		...
		foreach( $hosted_fonts as $family ){
			...
			$css .= Dalmatian_Blog_Google_Local::init( $family )->get_css();
		}

		// If we've got CSS, add to the footer.
		if ( $css ) {
			echo '<style id="dalmatian-blog-local-webfonts">' . $css . '</style>'; // WPCS: XSS ok.
		}
	}
}